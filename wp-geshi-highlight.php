...
// Search all <pre><code class="language-*"></code></pre> occurrences. Store them globally.
...
function wp_geshi_add_css_to_head() {
	...
	// (without a trailing slash) and add a trailing slash.
...
function wp_geshi_tiny_mce_settings_filter($settings) {
	// Upon switching between the "text" (raw) view and the "HTML"
	// (TinyMCE-based) view the TinyMCE editor may modify content.
	// This filter instructs TinyMCE to leave <pre> elements alone
	// when they have non-standard attributes as used by this plugin,
	// such as `lang` and `cssfile` and `lang`.
	...
	// From the TinyMCE docs: "When adding a new attribute by specifying an
	// existing element rule (e.g. img), the entire rule for that element
	// is over-ridden" -- therefore, allow `pre[*]` which should be less
	// strict than any other specification string for the `pre` element.

	// If other plugins modify extended_valid_elements then there is
	// no conflict unless the `pre` tag behavior gets modified. Upon
	// conflict the filter ID matters I believe.
